...
//creation of local variables to store data
let rooms = [];
...
// let date_regex1 = /^(?:(?:1[6-9]|[2-9]\d)?\d{2})(?:(?:(\/|-|\.)(?:0?[13578]|1[02])\1(?:31))|(?:(\/|-|\.)(?:0?[13-9]|1[0-2])\2(?:29|30)))$|
...
////Get syntaxes for all the API endPoints as per requirement
...
//fetch data on all the bookings
...
//Post scripts and logic for bookings

//create rooms for booking
...
app.post("/createBooking", function (req, res) {
    ...
    booking.id = uniqid();
    //Condition checking the name of the customer
    ...
    if(req.body.endTime){
        ...
        res.status(400).json({ output: 'Please specify End time for booking.'})
    }

    //filter rooms available for booking
    //Here we are parsing through the available data and comparing them to the request for output
    ...
    if(availableRooms.length == 0){res.status(400).json({ output: 'No Available rooms on required Date and Time'})}
   else{
